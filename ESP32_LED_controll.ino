...
#define led_pin_1 4 // data pin used to send control signal to led strip
#define led_pin_2 16 // data pin used to send control signal to the second led strip
#define led_pin_3 15 // data pin used to send control signal to third led strip
#define led_pin_4 5 // data pin used to send control signal to fourth led strip

#define clock_pin_1 2
#define clock_pin_2 5
...
CRGB leds1[NUM_LEDS];       // Array to store values for each led in the led strip. 
CRGB leds2[NUM_LEDS];
CRGB leds3[NUM_LEDS];
CRGB leds4[NUM_LEDS];
...
const unsigned short int LIGHT_SENSOR_PIN = 18;                   // Declares the pin to be used for the input from the photocell
...
unsigned short int TILT_SENSOR_INACTIVITY_COUNTER = 0;       // Stores the counters for inactivity from all tilt sensors 
bool ACTIVE;
...
void setup() {
  ...
  pinMode(LIGHT_SENSOR_PIN, INPUT); // Input pin used to recieve data about movement of the sytem
  ...
  FastLED.addLeds<LED_TYPE,led_pin_3,clock_pin_1, COLOR_ORDER>(leds3, NUM_LEDS).setCorrection(TypicalLEDStrip);   //Initialize third ledstrip
  FastLED.addLeds<LED_TYPE,led_pin_4,clock_pin_1, COLOR_ORDER>(leds4, NUM_LEDS).setCorrection(TypicalLEDStrip);   //Initialize fourth ledstrip
...
void loop() {
  ...
  int LIGHT_INTENSITY = analogRead(LIGHT_SENSOR_PIN);             // Current reading from the photocell
  ...
  if (LIGHT_INTENSITY < 2000 && ACTIVE == true) {                                   // Check if the sensor registers lights or not
    if (ESP_BT.available())                         //checks if there is a bluetooth connection
      ...
      gCurrentPatternNumber1 = 8;
      gCurrentPatternNumber2 = 8;
  }

    TILT_SENSOR_CURRENT_LIST = (round(analogRead(TILT_SENSOR_PINS[0])/4095));    // Reads all the tilt pins and reduces to binary results in a new list
